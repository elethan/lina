import { getMachineClinicalStatusLabel, isNonClinicalMachineStatus } from '../../lib/machine-clinical-status'
...
export function displayAssetStatus(status: string) {
    return isNonClinicalMachineStatus(status)
        ? getMachineClinicalStatusLabel(status)
        : status
}
